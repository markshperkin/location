 ...
 */
class MainActivity : AppCompatActivity(), OnMapReadyCallback, SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onMapReady(gMap: GoogleMap) {
        Toast.makeText(this, "Creating a new map", Toast.LENGTH_SHORT).show()

        googleMap = gMap // Assign the GoogleMap object when it's ready
        // This method will be called once the map is ready
    }
    // Function to update the map with the given coordinates
    // Function to update the map with the received location
    private fun updateMapWithLocation(location: Location) {
        Toast.makeText(this, "Updating map with new location", Toast.LENGTH_SHORT).show()

        googleMap?.let {
            val currentLatLng = LatLng(location.latitude, location.longitude)
            it.clear() // Clear existing markers
            it.addMarker(
                MarkerOptions()
                    .position(currentLatLng)
                    .title("Marker in current location")
            )
            it.moveCamera(CameraUpdateFactory.newLatLngZoom(currentLatLng, 15f))
        }
    }
    ...
    private inner class ForegroundOnlyBroadcastReceiver : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (location != null) {
                ...
                currentLocation = location // Update the global variable with the latest location
                // Call the function to update the map with the new location
                googleMap?.let { updateMapWithLocation(location) } // Call the function to update the map
                Toast.makeText(context, "Location received: ${location.toText()}", Toast.LENGTH_SHORT).show()
            }
        }
    }
}
